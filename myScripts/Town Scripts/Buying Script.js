	...
	var isButtonCreated4 : boolean = GUI.Button(myRect4, bta);
		if (isButtonCreated4) {
			if(stopBuy==false){
	    		...
	    		//priceText.text = price + " G";
	    		item1Count += 1;
	    		sanaDia.text = '"Hmm these look like a good deal today. I can sell these back for twice as much."';
	    		if(item1Count > 2){
	    			item1Count = 2;
	    			itemQ1.text = item1Count.ToString();
	    		}
	...
	var isButtonCreated6 : boolean = GUI.Button(myRect6, bta2);
		if (isButtonCreated6) {
			if(stopBuy==false){
	    		...
	    		//priceText.text = price + " G";
	    			item2Count += 1;
	    			sanaDia.text = '"Hmm these look good."';
	    		if(item2Count > 2){
	    			item2Count = 2;
	    			itemQ2.text = item2Count.ToString();
	    		}
...
var bta3 = addButtonTexture;
	...
	var isButtonCreated8 : boolean = GUI.Button(myRect8, bta3);
		if (isButtonCreated8) {
			if(stopBuy==false){
				item3Count += 1;
				sanaDia.text = '"These are a great deal I should take some of these."';
	    		if(item3Count > 2){
	    			item3Count = 2;
	    			itemQ3.text = item3Count.ToString();
	    		}
...
//**************************************************************sell
	...
	if(stopBuy == false){
		//add 
		var iceCreamPrice = item1Count * 30;	
		var cakePrice = item2Count * 50;
		var bagelPrice = item3Count * 20;
		var cakeRollPrice = item4Count * 70;
		var redCakePrice = item5Count * 100;
		var chocoCakePrice = item6Count *120;
