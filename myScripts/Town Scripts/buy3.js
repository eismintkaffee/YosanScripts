	...
	var isButtonCreated4 : boolean = GUI.Button(myRect4, bta);
		if (isButtonCreated4) {
			if(stopBuy==false){
	    		...
	    		//priceText.text = price + " G";
	    		item1Count += 1;
	    		sanaDia.text = '"I dont think I can afford this store. Lets come back later"';
	    		if(item1Count > 2){
	    			item1Count = 2;
	    			itemQ1.text = item1Count.ToString();
	    		}
...
//**************************************************************sell
	...
	if(stopBuy == false){
		//add 
		var iceCreamPrice = item1Count * 500;	
		var cakePrice = item2Count * 550;
		var bagelPrice = item3Count * 400;
		var cakeRollPrice = item4Count * 1670;
		var redCakePrice = item5Count * 2100;
		var chocoCakePrice = item6Count *2500;
		
		totalPurchase = (iceCreamPrice + cakePrice + bagelPrice + redCakePrice + chocoCakePrice + cakeRollPrice) * 1.03 ;
